...
// Section headings for every export path. The backend has no translations of
// its own, so the labels travel with the request and it falls back to English
// only if they are missing.
function sectionLabels(t) {
  return {
    summary: t("sections.summary"),
    key_data_points: t("sections.keyDataPoints"),
    action_items: t("sections.actionItems"),
    red_flags: t("sections.redFlags"),
    report_title: (t("title") || "").replace(/<\/?\d+>/g, "").replace(/\s+/g, " ").trim(),
  };
}

function buildTxtReport(filename, result, labels) {
  const sections = [
    [labels.summary, result.summary],
    [labels.key_data_points, result.key_data_points],
    [labels.action_items, result.action_items],
    [labels.red_flags, result.red_flags],
  ];
  const lines = [`${labels.report_title}: ${filename}`, "Generated by Workblox — Torres Tech Remote", ""];
...
export default function DocAnalyzer() {
  ...
  function downloadTxt() {
    const txt = buildTxtReport(filename, result, sectionLabels(t));
  ...
  async function sendEmail(e) {
    ...
    try {
      await post("/api/doc/email", { email: emailAddr, filename, result, labels: sectionLabels(t) });
      setEmailSent(true);
    } catch (err) {

...
export default function SocialMediaGenerator() {
  const { t, i18n } = useTranslation("socialMedia");
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const result = await call(() => post("/api/biz/social-media", { topic, business_name: businessName, platforms, tone, goal, language: i18n.language }));
      ...
      <form onSubmit={handleSubmit} className="no-print" style={{ display: "flex", flexDirection: "column", gap: "0.75rem", marginBottom: "1.5rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
          ...
          <select value={goal} onChange={(e) => setGoal(e.target.value)} disabled={loading} style={{ ...inputStyle, background: "var(--surface)", cursor: "pointer" }}>
            {["engagement", "awareness", "leads", "sales", "education"].map(opt => <option key={opt} value={opt}>{t(`goalOptions.${opt}`)}</option>)}
          </select>
        </div>
        ...
        <>
          ...
          {(data.posts || []).map((p, i) => (
            <div key={i} className="result-card">
              ...
              {p.best_time_to_post && (
                <p style={{ fontSize: "0.78rem", color: "var(--text-hint)", marginTop: "0.5rem" }}>{t("bestTime", { time: p.best_time_to_post })}</p>
              )}
            </div>
          ))}
          {data.content_tips?.length > 0 && (
            <div className="result-card">
              <p className="result-label">{t("contentTips")}</p>
              <ul className="section-list">{data.content_tips.map((tip, i) => <li key={i}>{tip}</li>)}</ul>
            </div>
          )}
          <ReportToolbar

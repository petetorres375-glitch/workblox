...
def _generate_pdf(subject, txt_content, language=None):
    ...
    # Direction is decided once for the whole report so that headings, rules
    # and body all agree, even on a line that happens to be mostly Latin
    # (a company name, a date).
    body_align = align_for(txt_content, language)

    def cell_ln(h, text, font_size=9, bold=False, color=(31, 41, 55)):
        pdf.set_x(pdf.l_margin)
        pdf.set_font(font, "B" if bold else "", font_size)
        pdf.set_text_color(*color)
        pdf.multi_cell(W, h, text, align=body_align)
        pdf.set_x(pdf.l_margin)
    ...
    while i < len(lines):
        ...
        if next_is_sep and line.strip():
            ...
            pdf.cell(W, 7, line.strip(), align=body_align)
            pdf.ln(7)
            pdf.set_x(pdf.l_margin)

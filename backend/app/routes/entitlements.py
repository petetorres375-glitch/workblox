...
def put_entitlements():
    ...
    valid_keys = {t.key for t in Tool.query.filter(Tool.key.in_(tool_keys)).all()}
    unknown = set(tool_keys) - valid_keys
    if unknown:
        return jsonify({"error": f"Unknown tool key(s): {', '.join(sorted(unknown))}"}), 400

    set_entitlements(user_id, valid_keys, source="self_service")
    return jsonify({"tool_keys": sorted(valid_keys)})
